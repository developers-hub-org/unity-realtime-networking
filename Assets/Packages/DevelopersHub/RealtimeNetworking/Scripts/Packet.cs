...
namespace DevelopersHub.RealtimeNetworking
{
    public class Packet : IDisposable
    {
        ...
        public void InternalSendInitialize()
        {
            if (compress)
            {
                buffer = Compress(buffer.ToArray()).ToList();
            }
            buffer.InsertRange(0, BitConverter.GetBytes(compress));
        }

        public void InternalReceiveInitialize()
        {
            if (decompressed) { return; }
            decompressed = true;
            if (ReadBool())
            {
                byte[] decompressedData = Decompress(ReadBytes(UnreadLength(), false));
                buffer = decompressedData.ToList();
                readableBuffer = decompressedData;
                readPos = 0;
            }
        }
